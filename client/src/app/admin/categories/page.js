...
export default function AdminCategoriesPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      if (editingId) {
        ...
        await apiJson("/categories", { method: "POST", body: JSON.stringify(payload) });
      }
      setShowForm(false);
      load();
    } catch (err) {
  ...
  async function onDelete(id) {
    if (!confirm("Delete this category? Any products still assigned to it will keep the reference but won't show a category name until reassigned.")) return;
    try {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-heading text-lg font-bold text-white">Categories ({categories.length})</h2>
        <button
          onClick={startAdd}
      ...
      {loading ? (
        ...
        <p className="text-white/50">No categories yet — add one to get started.</p>
      ) : (
              ...
              {categories.map((c) => (
                <tr key={c._id} className="border-t border-white/10 text-white/80">
                  ...
                  <td className="p-3 text-right">
                    <button onClick={() => startEdit(c)} className="mr-3 text-cyan-300 hover:underline">
                      Edit
                    </button>
                    <button onClick={() => onDelete(c._id)} className="text-red-400 hover:underline">
                      Delete
                    </button>
                  </td>
                </tr>
